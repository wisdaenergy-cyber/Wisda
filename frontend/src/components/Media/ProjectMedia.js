...
export default function ProjectBannerSection({ title }) {
    ...
    if (selectedFile) {
      ...
      try {
        const res = await axios.post(
          `${process.env.NEXT_PUBLIC_API_URL}/api/v1/banner/project/upload`,
          formData,
          {
            headers: {
...
try {
    ...
    {
        const res=await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/banner/project/banner`)
