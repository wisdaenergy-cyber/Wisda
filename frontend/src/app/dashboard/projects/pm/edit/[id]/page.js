...
export default function EditBlogPage() {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        ...
        const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/project/pm/${blogId}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/project/pm/update`, formData, {
        headers: {
  ...
  return (
    <div className="min-h-screen bg-[#E2DCD0] p-6">
      ...
      >
        <h2 className="text-2xl font-semibold text-[#278188]">Edit PM Project</h2>
