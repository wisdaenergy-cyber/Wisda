...
export default function BlogBannerSection({ title }) {
    ...
    if (selectedFile) {
      ...
      try {
        const res = await axios.post(
          `${process.env.NEXT_PUBLIC_API_URL}/api/v1/banner/blog/upload`,
          formData,
          {
            headers: {
...
try {
  ...
  const imageSrc = preview || (banner ? banner : "/AboutBanner.jpg");
